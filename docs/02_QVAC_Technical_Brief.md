...
There is no biosignal modality, no neural inference capability, and no BCI plugin scaffolding. The QVAC SDK is JavaScript-based and expects plugins to conform to a JS-native plugin architecture with type-safe APIs.
...
The Phase 1 service implements endpoints mirroring QVAC's OpenAI-compatible patterns. Sketch (final specifications to be agreed with QVAC team):

```
POST /v1/eeg/ingest
   Streams EEG data from connected hardware or file source
   Returns session ID and real-time quality score

POST /v1/eeg/biomarkers
   Body: { session_id, biomarker_set }
   Returns: { biomarkers: [...], confidence_scores: {...} }

POST /v1/bci/classify
   Body: { session_id, paradigm, classifier_type }
   Returns: { prediction, confidence, latency_ms }

POST /v1/eeg/compare
   Body: { session_a_id, session_b_id, metrics }
   Returns: { effect_sizes, p_values, plain_english_summary }
```

The service runs on `localhost:PORT` (default 8765, configurable). QVAC SDK applications discover it through a standard configuration file.
...
skybrain-qvac-bci/
├── README.md
├── LICENSE (Apache 2.0 or MIT per QVAC preference)
├── docs/
│   ├── quickstart.md
│   ├── api-reference.md
...
The proprietary SkyBrain SDK is installed as a dependency, not bundled in the open-source repo. This keeps the boundary clear: open layer is the integration code, proprietary layer is the actual signal processing intelligence.
...
- **Week 2:** Initial technical alignment call with QVAC team, agreement on API surface and plugin conformance
- **Week 4:** Repository scaffolding live, CI/CD running, first endpoints functional
- **Week 8:** All Phase 1 API endpoints operational, basic Cognitive Edge integration working in dev
- **Week 10:** Cross-platform compatibility verified on macOS/Linux/Windows, Android via companion app
- **Week 12:** Demo recording produced, documentation complete, Phase 1 review
...
Listed honestly because we'd rather surface these now than discover them later:

1. **License preference for the open-source layer.** Apache 2.0 vs MIT vs another permissive license? We are flexible.
2. **Plugin architecture conventions.** Are there existing QVAC plugin examples we should mirror? The docs at docs.qvac.tether.io give the basics; we'd appreciate review of our manifest before publication.
3. **Cognitive Edge as a QVAC reference app.** Does QVAC team want this submitted through a particular process? Branding alignment expectations?
4. **WDK integration.** Should the integration include WDK for any payment flows (e.g., participants in the optional consent-driven data marketplace receiving USDT)? We are open to scoping this in if it advances QVAC's broader vision.
5. **Holepunch / Pears P2P primitives.** Phase 2 explores delegated inference. Are there Holepunch reference implementations we should align with?
6. **iOS native runtime.** Phase 2 targets iOS via Expo. Is there a preferred QVAC mobile pattern beyond Expo we should evaluate?
