...
SkyBrain Neurotech builds EEG/BCI infrastructure: a proprietary Python SDK that turns raw brain-signal recordings into 50+ biomarkers and runs 7 BCI paradigms across 5 Bayesian classifiers. Tether's QVAC ecosystem ships local-first AI (LLM, embeddings, ASR, TTS, OCR, NMT, image gen) via an OpenAI-compatible HTTP server and a JS SDK — but lacks any biosignal layer. We are open-sourcing the bridge between SkyBrain's SDK and QVAC under Apache 2.0 so QVAC consumers (Cognitive Edge, QVAC Health, QVAC MedPsy) can ground their reasoning in physiological brain state without raw EEG ever leaving the user's device. Phase 1 (current) is a localhost FastAPI service in Python; Phase 2 (later) will port the core to native JavaScript as an npm package.
